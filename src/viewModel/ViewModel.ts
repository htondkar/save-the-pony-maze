...
export class ViewModel {
	...
	public getPathToTarget() {
		...
		return mazeModel.getShortestPath(this.mazeState.pony[0], this.mazeState['end-point'][0])
	}
	...
	public async createNewGame(options: GameOptions) {
		...
		try {
			...
			alert('failed to create a new game. ' + typeof error === 'string' ? error : error.message)
		} finally {
	...
	public async syncGameState(mazeId: string) {
		try {
			...
			alert('failed to update the game game. ' + typeof error === 'string' ? error : error.message)
		}
	}
	...
	public async movePonyTowardsTarget() {
		...
		try {
			...
			const moveResult = await this.networkApi.movePony(nextMove, this.currentMazeState.maze_id)
